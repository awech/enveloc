...
Before using *enveloc* I strongly recommend familiarizing yourself with Obspy.
...
Seismic data is input as an [Obspy Stream](https://docs.obspy.org/packages/autogen/obspy.core.stream.Stream.html).
...
!!! note
    1. If you are trying to locate a single window, make sure to trim off any edge effects, such as a pre-filter taper,
    ...
    2. Be careful not to make the window length too small. In general the window length should be longer than the largest
...
Each [Trace](https://docs.obspy.org/packages/autogen/obspy.core.trace.Trace.html), `tr`, within the Stream
object must also contain its associated latitude and longitude (in decimal degrees) in the form of an Obspy
AttribDict named `coordinates`, which itself is a key for Trace's AttribDict `stats`. This could
be extracted from an [Obspy Inventory](https://docs.obspy.org/packages/obspy.core.inventory.html#module-obspy.core.inventory):
...
constant velocity provided. More details provided in the [XCOR class reference](xcor.md).
