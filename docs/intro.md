...
These codes have been used operationally at the
[Pacific Northwest Seismic Network](https://pnsn.org/) since 2009 to automatically catalog
[tremor there](https://pnsn.org/tremor). This origin leads to the assumption that the package serves
as a tremor detector, which is a mischaracterization. The code *can* be used that way, but it doesn't
have to be.
...
correlation between two stations and the correlation value associated with the predicted differential traveltime:

<figure markdown="span">
  ![enveloc method](enveloc_method.png){ width="300" }
  <figcaption>
  Example normalized cross correlogram between the envelopes of stations \(i\) and \(j\)
  as a function of lag time \(\delta t\). The correlogram \(C_{ij}\) evaluated at the differential
  <em>S</em>-wave travel time \(\delta t_{ij}(x^{grid})\) results in the predicted cross correlation for
  that grid node \(C_{ij}[\delta t_{ij}(x^{grid})]\). The algorithm minimizes the vertical correlation
  distance by minimizing the sum over station pairs of
  \(\sum[C_{ij}^{max} - C_{ij}[\delta t_{ij}(x^{grid})]]\) inversely weighted by its uncertainty
  \(\Delta C_{ij}^{max}\). See
...
3. **Tectonic tremor.**
   *enveloc* was originally designed to search through continuous data and attempt a location on overlapping 5-minute
...
I find there's no substitute for looking at the data.

## Limitations

Envelope cross correlation is not always the best method to use, and using *enveloc* has its drawbacks:

1. Creating envelopes inherently degrades temporal resolution, which will affect the resulting spatial
   resolution. Horizontal uncertainty is briefly discussed above in the [Uncertainty](#uncertainty)
...
2. Because location is based on a grid search, the code will always provide a solution. As discussed in the
   [Detection](#detection) section above, this means care must be taken when interpreting the output.
3. The code does not work on signals with little amplitude variation. In this case there's little structure for the
   envelopes to correlate.
4. Issues can arise when the start or end of a window is in the middle of a high-amplitude event. In this case
...
5. **AUTOMATION IS HARD.** *enveloc* has a lot of internal checks and quality-control measures to try and handle the most
   conspicuous gotchas, but nothing is bulletproof. Even traditional earthquake locations (which we've known how to
   do for a long time now) require a human analyst. Trying to automate the detection and location of messy seismicity
